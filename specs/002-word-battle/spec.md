...
### User Story 4 - ボス戦とゲームクリア (Priority: P4)
...
- **FR-021**: 報酬選択時の文字選択と装備先選択は全てキーボード入力のみで行えなければならない（UIボタンコンポーネントを使用しない）。敵の各文字に数字キー（1,2,3...）を割り当て、装備先は武器キー・盾キーで選択する
...
- **FR-019**: 戦闘時のプレイヤー操作（攻撃・防御・リスタート等）は全てキーボード入力のみで行えなければならない（マウスクリック用UIボタンを設置しない）
- **FR-020**: 現在選択可能な操作の一覧（例: Attack[武器キー] / Defend[盾キー] / Restart[Enter] など）を常時表示する専用UIパネルを表示し、ターン状態に応じて動的に更新しなければならない

### Key Entities

- **プレイヤー**: 名前（英単語）から生成される武器・盾を持つ。HP、現在の装備文字列を持つ
